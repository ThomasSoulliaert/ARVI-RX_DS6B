...
| Sorties | `normal`, `suspected_opacity`, `uncertain` (garde-fou, pas un échec) |
...
Point de jonction clé : l'application appelle `run_prediction` → `predict_with_model`, qui tente le vrai VLM et retombe sur le jouet sans casser la plomberie.
...
Le taux de JSON valide observé est de **100 %** sur les trois prompts : le socle d'intégration est robuste (mais la validité formelle ne garantit pas la correction du contenu — voir §17, HT).
...
- **`improved_v2`** (v3) : même procédure, mais les exemples **ne portent plus de valeurs de confiance imitables** et une consigne explicite interdit de recopier les nombres des exemples.
...
`src/inference.py` implémente la vraie inférence VLM sur `google/medgemma-4b-it` (chargement unique en cache, warmup en thread, extraction robuste du bloc JSON). MedGemma est *gated* (licence + token). En l'absence de GPU/token, `predict_with_model` retombe sur un **modèle jouet déterministe** pour valider la plomberie ; en évaluation RSNA, une **garde anti-fallback** interrompt le run plutôt que de produire des métriques calculées sur le jouet. Conditions d'accès et licences : §13.
...
Voir [`licences_et_sources.md`](licences_et_sources.md) : code MIT ; MedGemma *gated* (Health AI Developer Foundations, non redistribué, non-dispositif médical) ; RSNA à accès Kaggle contrôlé, non redistribuable, dé-identifié (pixels seuls) ; licences des bibliothèques. À revérifier avant tout rendu.
...
Le prototype démontre une **méthode**, pas un modèle spectaculaire : chaîne complète et traçable, socle d'intégration robuste (100 % JSON valides et warnings), et surtout une **amélioration mesurée honnêtement** — y compris son échec intermédiaire (`improved`) et les compromis de sa correction (`improved_v2`). Perspectives, par priorité :
